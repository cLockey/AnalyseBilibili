...
class Login():  
    ...
    def get_images(self, bg_filename = 'bg.jpg', fullbg_filename = 'fullbg.jpg'):  
        ...
        style_str = bg[0].get('style')
        left_idx = str(style_str).find('\"')
        right_idx = str(style_str).rfind('\"')
        print("leftIdx: "+str(left_idx), ", rightIdx: "+str(right_idx))
        bg_url = style_str[left_idx+1: right_idx].replace('webp', 'jpg')
        print("url: ", bg_url)
        fullbg_style_str = fullbg[0].get('style')
        left_idx = str(fullbg_style_str).find('\"')
        right_idx = str(fullbg_style_str).rfind('\"')
        print("leftIdx: "+str(left_idx), ", rightIdx: "+str(right_idx))
        fullbg_url = fullbg_style_str[left_idx+1: right_idx].replace('webp', 'jpg')
    ...
    def login(self):  
        ...
        # 获取缺口位置  
        gap = self.get_gap(fullbg_img, bg_img) + random.randint(-2,2)   #过于准确貌似会失败，加个随机偏移量
        print('缺口位置', gap)  
